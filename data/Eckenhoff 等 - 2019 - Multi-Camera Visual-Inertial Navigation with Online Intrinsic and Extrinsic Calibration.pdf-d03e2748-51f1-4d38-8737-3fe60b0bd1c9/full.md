...
- We validate the proposed mc-VINS on real multicamera visual-inertial sensor platforms using different camera configurations and in various environments.
...
$$
\mathbf {x} _ {I} = \left[ _ {G} ^ {I} \bar {q} ^ {\top} \mathbf {b} _ {\omega} ^ {\top} ^ {G} \mathbf {v} _ {I} ^ {\top} \mathbf {b} _ {a} ^ {\top} ^ {G} \mathbf {p} _ {I} ^ {\top} \right] ^ {\top} \tag {1}
$$

where ${}_{G}^{I}\bar{q}$ is the JPL unit quaternion [25] associated with the rotation matrix, ${}_{G}^{I}\mathbf{R}$ , which rotates vectors from the global frame of reference $\{G\}$ into the local frame $\{I\}$ of the IMU, $\mathbf{b}_{\omega}$ and $\mathbf{b}_a$ are the gyroscope and accelerometer biases which corrupt the IMU measurements, ${}^{G}\mathbf{p}_{I}$ is the position of the IMU expressed in the global frame, and ${}^{G}\mathbf{v}_{I}$ is the corresponding velocity. Associated with this navigation state we also define the corresponding error state as:

$$
\delta \mathbf {x} _ {I} = \left[ ^ {I} \delta \boldsymbol {\theta} _ {G} ^ {\top} \delta \mathbf {b} _ {\omega} ^ {\top} ^ {G} \delta \mathbf {v} _ {I} ^ {\top} \delta \mathbf {b} _ {a} ^ {\top} ^ {G} \delta \mathbf {p} _ {I} ^ {\top} \right] ^ {\top} \tag {2}
$$
...
$$
\mathbf {x} _ {I} = \hat {\mathbf {x}} _ {I} \boxplus \delta \mathbf {x} _ {I} \tag {3}
$$
...
$$
\bar {q} \approx \left[ \begin{array}{c} \frac {1}{2} \delta \boldsymbol {\theta} \\ 1 \end{array} \right] \otimes \hat {\bar {q}} \tag {4}
$$
...
$$
\mathbf {x} _ {c l} = \left[ \begin{array}{l l} I (t _ {k}) & G \\ G & I (t _ {k}) \end{array} \right] \dots \left. \begin{array}{l l} I (t _ {k - M + 1}) & G \\ G & I (t _ {k - M + 1}) \end{array} \right] ^ {\top} \tag {5}
$$
...
$$
\mathbf {x} _ {c i} = \left[ \begin{array}{l l l} C _ {i} & \bar {q} ^ {\top} & C _ {i} \\ I & \bar {q} ^ {\top} & t _ {b} \end{array} \right] ^ {\top} \tag {6}
$$

$$
\boldsymbol {\zeta} _ {i} = \left[ f _ {x i} f _ {y i} p _ {x i} p _ {y i} \mathbf {d} _ {i} ^ {\top} \right] ^ {\top} \tag {7}
$$

where ${}^i t_b$ is the time offset between camera $i$ and the base camera, $f_{xi}$ , $f_{yi}$ represent the focal lengths, $p_{xi}$ , $p_{yi}$ denote the location of the principal point, and $\mathbf{d}_i$ refers to the vector of distortion parameters whose length/definition depends on the camera model being used (see [27]). For the base camera, however, we store its temporal misalignment with respect to the IMU, ${}^b t_I$ as the relative time offset to itself is zero.

# B. IMU Propagation

As the sensor platform navigates in the space, the IMU measures both the angular velocity, $\omega_{m}$ , and local linear acceleration, $\mathbf{a}_{m}$ , which are utilized for propagation:

$$
\boldsymbol {\omega} _ {m} = \boldsymbol {\omega} + \mathbf {b} _ {\omega} + \mathbf {n} _ {\omega} \tag {8}
$$

$$
\mathbf {a} _ {m} = ^ {I} \mathbf {a} + _ {G} ^ {I} \mathbf {R} ^ {G} \mathbf {g} + \mathbf {b} _ {a} + \mathbf {n} _ {a} \tag {9}
$$

where $\omega$ and $^I\mathbf{a}$ represent the true local angular velocity and local linear acceleration of the IMU, while $\mathbf{n}_{\omega}$ and $\mathbf{n}_a$ are continuous-time Gaussian white noises, and ${}^{G}\mathbf{g}\approx$ $[0 0 9.81]^{\top}$ is the known global gravity.

It is important to note that due to hardware latency of onboard processing, the time reported by the base camera will differ from the same time expressed in the IMU's clock. We consider a time ${}^{b}t$ as expressed in the base camera's clock, which is related to the same instant represented in the IMU clock, ${}^{I}t$ , by a time offset ${}^{b}t_{I}$ , i.e.,

$$
^ {I} t = ^ {b} t + ^ {b} t _ {I} \tag {10}
$$

As this time offset is unknown, we include it as a parameter in our state vector to be estimated.

With the estimate of this time offset ${}^b\hat{t}_I$ , whenever we receive the $(k + 1)$ -th image with reported time ${}^b t_{k + 1}$ , we perform propagation of our IMU up to the estimated time of the image as expressed in the IMU clock [see (10)]: ${}^I\hat{t}_{k + 1} = {}^b t_{k + 1} + {}^b\hat{t}_I$ . Specifically, we propagate the IMU from its current time ${}^I\hat{t}_k$ (actually the estimate of the IMU time for $k$ -th image at the time of the previous propagation,

before update) up to this new time by processing all IMU measurements $\mathcal{I}$ collected over the time interval $[^{I}\hat{t}_{k},^{I}\hat{t}_{k + 1}]$ , based on the conventional IMU dynamics whose integration yields the prediction function $\mathbf{f}(\cdot)$ [28]:

$$
\hat {\mathbf {x}} _ {I \left(^ {I} \hat {t} _ {k + 1}\right) | k} = \mathbb {E} \left[ \mathbf {f} \left(\mathbf {x} _ {I \left(^ {I} \hat {t} _ {k}\right))}, \mathcal {I}, \mathbf {n} _ {I}\right) \right] = \mathbf {f} \left(\hat {\mathbf {x}} _ {I \left(^ {I} \hat {t} _ {k}\right) | k}, \mathcal {I}, \mathbf {0}\right)
$$

where the $\hat{\mathbf{x}}_{|k}$ notation denotes the estimate for variable $\mathbf{x}$ given all measurements up to the $k$ -th image, while $\mathbf{n}_I$ is the stacked IMU noises. Based on this prediction function, we compute the state-transition matrix across the interval $\Phi_k$ as well as the corresponding noise covariance $\mathbf{Q}_k$ as in [7, 8]. Once computed, the covariance after propagation $\mathbf{P}_{k + 1|k}$ can be calculated as:

$$
\mathbf {P} _ {k + 1 \mid k} = \left[ \begin{array}{c c} \boldsymbol {\Phi} _ {k} \mathbf {P} _ {I I, k \mid k} \boldsymbol {\Phi} _ {k} ^ {\top} + \mathbf {Q} _ {k} & \boldsymbol {\Phi} _ {k} \mathbf {P} _ {I S, k \mid k} \\ \mathbf {P} _ {S I, k \mid k} \boldsymbol {\Phi} _ {k} ^ {\top} & \mathbf {P} _ {S S, k \mid k} \end{array} \right] \tag {11}
$$
...
After propagating to time step $k + 1$ , we only have a state estimate of the IMU at the estimated time ${}^I\hat{t}_{k + 1}$ while we actually need to express all camera measurements as a function of the IMU pose at the true time ${}^I t_{k + 1}$ . To accomplish this, we utilize stochastic cloning [29]. Specifically, consider our current state $\mathbf{x}$ with covariance $\mathbf{P}$ which we wish to augment with another state that can be written as a function $\mathbf{g}(\mathbf{x})$ . The augmented state mean and covariance of the corresponding Gaussian can be computed as: $\mathcal{N}\left(\left[ \begin{array}{l}\hat{\mathbf{x}}\\ \mathbf{g}(\hat{\mathbf{x}}) \end{array} \right],\Psi \mathbf{P}\Psi^{\top}\right)$ , where $\Psi = \left[\frac{\mathbf{I}}{\frac{\partial\mathbf{g}(\hat{\mathbf{x}}\boxplus\delta\mathbf{x})\boxminus\mathbf{g}(\hat{\mathbf{x}})}{\partial\delta\mathbf{x}}}\right]$ . Using this methodology, the following linearized stochastic cloning is performed to create an estimate of the IMU at this true time in a manner analogous to [22]:

$$
{ } ^ { G } \mathbf { p } _ { I ( ^ { I } t _ { k + 1 } ) } \approx { } ^ { G } \mathbf { p } _ { I ( ^ { I } t _ { k + 1 } ) } + { } ^ { G } \mathbf { v } _ { I ( ^ { I } t _ { k + 1 } ) } \delta ^ { b } t _ { I } \tag {12}
$$

$$
\mathbf {\Lambda} _ {G} ^ {I \left(^ {I} t _ {k + 1}\right)} \mathbf {R} \approx \operatorname {E x p} \left(- \omega \delta^ {b} t _ {I}\right) _ {G} ^ {I \left(^ {I} \hat {t} _ {k + 1}\right)} \mathbf {R} \tag {13}
$$

Where $\mathrm{Exp}(\cdot)$ is the $SO(3)$ matrix exponential which maps a vector in $\mathbb{R}^3$ to a rotation matrix [30]. That is, because we can write the pose at the true time as a function of the pose at the estimated time, as well as the time offset error (both of which are contained in our state), we can augment our state to contain this new pose. The Jacobians for this cloning are given by:

$$
\frac {\partial^ {G} \delta \mathbf {p} _ {I \left(^ {I} t _ {k + 1}\right)}}{\partial^ {G} \delta \mathbf {p} _ {I \left(^ {I} \hat {t} _ {k + 1}\right)}} = \mathbf {I} _ {3}, \frac {\partial^ {G} \delta \mathbf {p} _ {I \left(^ {I} t _ {k + 1}\right)}}{\partial \delta^ {b} t _ {I}} = ^ {G} \hat {\mathbf {v}} _ {I \left(^ {I} \hat {t} _ {k + 1}\right)}
$$

$$
\frac {\partial^ {I \left(^ {I} t _ {k + 1}\right)} \delta \boldsymbol {\theta} _ {G}}{\partial^ {I \left(^ {I} \hat {t} _ {k + 1}\right)} \delta \boldsymbol {\theta} _ {G}} = \mathbf {I} _ {3}, \quad \frac {\partial^ {I \left(^ {I} t _ {k + 1}\right)} \delta \boldsymbol {\theta} _ {G}}{\partial \delta^ {b} t _ {I}} = \boldsymbol {\omega} _ {m} (^ {I} \hat {t} _ {k + 1}) - \hat {\mathbf {b}} _ {w} (^ {I} \hat {t} _ {k + 1})
$$
...
Consider a 3D feature, ${}^{G}{\mathbf{p}}_{f}$ ,which is captured by the $i$ -th camera at imaging time ${}^{b}{t}_{k}$ with respect to the base camera. In this work we track features extracted uniformly using FAST [31] and tracked independently for each camera's image stream using KLT [32], with outliers rejected via 8-point RANSAC. The perspective projection measurement function for this feature is given by:

$$
\mathbf {z} _ {k} = \mathbf {w} _ {i} \left(\boldsymbol {\Pi} \left(^ {C _ {i} \left(^ {b} t _ {k}\right)} \mathbf {p} _ {f}\right), \boldsymbol {\zeta} _ {i}\right) + \mathbf {n} _ {k}, \boldsymbol {\Pi} \left(\left[ \begin{array}{c} x \\ y \\ z \end{array} \right]\right) = \left[ \begin{array}{c} \frac {x}{z} \\ \frac {y}{z} \end{array} \right]
$$

where $\mathbf{w}_i(\cdot)$ is the function mapping the normalized image coordinates onto the image plane based on the camera intrinsics $\zeta_i$ and the camera model used (e.g., radial-tangential or fisheye [33]), and $C_i(^b t_k)\mathbf{p}_f = [xyz]^\top$ is the position of the feature expressed in camera $i$ 's frame at true time $^b t_k$ :

$$
{ } ^ { C _ { i } \left( ^ { b } t _ { k } \right) } \mathbf { p } _ { f } = { } _ { I } ^ { C _ { i } } \mathbf { R } _ { G } ^ { I \left( ^ { b } t _ { k } \right) } \mathbf { R } \left( ^ { G } \mathbf { p } _ { f } - ^ { G } \mathbf { p } _ { I \left( ^ { b } t _ { k } \right) } \right) + { } ^ { C _ { i } } \mathbf { p } _ { I } \quad ( 1 4 )
$$

Letting $\mathbf{a} = \boldsymbol{\Pi}\left(C_i^{\left(b t_k\right)}\mathbf{p}_f\right)$ denote the normalized image coordinates, we note that when computing Jacobians for this function, dependence on the intrinsic comes from the image plane mapping function $\mathbf{w}_i$ :

$$
\frac {\partial \mathbf {z} _ {k}}{\partial \delta \boldsymbol {\zeta} _ {i}} = \frac {\partial \mathbf {w} _ {i} (\mathbf {a} , \boldsymbol {\zeta} _ {i})}{\partial \delta \boldsymbol {\zeta} _ {i}} \tag {15}
$$

while for other variables $\mathbf{v}$ , the dependency is from $C_i\left(^b t_k\right)\mathbf{p}_f$ :

$$
\frac {\partial \mathbf {z} _ {k}}{\partial \delta \mathbf {v}} = \frac {\partial \mathbf {w} _ {i} (\mathbf {a} , \boldsymbol {\zeta} _ {i})}{\partial \mathbf {a}} \frac {\partial \mathbf {a}}{\partial^ {C _ {i} (b t _ {k})} \delta \mathbf {p} _ {f}} \frac {\partial^ {C _ {i} (b t _ {k})} \delta \mathbf {p} _ {f}}{\partial \delta \mathbf {v}} \tag {16}
$$

In summary, rather than using the undistorted, normalized pixel coordinates as measurements (as is typically done), we utilize the raw image coordinates, allowing us to calibrate the intrinsic of each camera along with the rest of the state.
...
$$
\mathbf {r} = \mathbf {H} _ {x} \delta \mathbf {x} + \mathbf {H} _ {f} ^ {G} \delta \mathbf {p} _ {f} + \mathbf {n} \tag {17}
$$
...
$$
\begin{array}{l} \mathbf {N} ^ {\top} \mathbf {r} = \mathbf {N} ^ {\top} \mathbf {H} _ {x} \delta \mathbf {x} + \mathbf {N} ^ {\top} \mathbf {H} _ {f} ^ {G} \delta \mathbf {p} _ {f} + \mathbf {N} ^ {\top} \mathbf {n} (18) \\ \Rightarrow \mathbf {r} ^ {\prime} = \mathbf {H} _ {x} ^ {\prime} \delta \mathbf {x} + \mathbf {n} ^ {\prime} (19) \\ \end{array}
$$

where the transformed noise, $\mathbf{n}^{\prime}$ , has covariance $\mathbf{N}^{\top}\mathbf{R}\mathbf{N}$ . As this transformed measurement is only a function of variables in our state, it can be utilized in the EKF update.

![](images/80ed58e1c1235de3a06c8972bac335e88f03d3a448323f24e03b0fc7fd25f5a9.jpg)  
...
In particular, assuming the measured time reported by the $i$ -th camera as $^t k$ , to express this time in the base camera's clock, we must compensate for the time offset, i.e., $^b t_k = ^t k + ^t b$ . Let $^b t_1$ and $^b t_2$ denote the times for the bounding base camera/IMU clones between which $^b\hat{t}_k = ^i t_k + ^i\hat{t}_b$ falls. We linearly interpolate the cloned poses at $^b t_1$ and $^b t_2$ to find the pose at the measurement time:

$$
\underset {G} {I ^ {(b} t _ {k)}} \mathbf {R} = \operatorname {E x p} \left(\lambda_ {k} \operatorname {L o g} \binom {I ^ {(b} t _ {2})} {G} \mathbf {R} _ {I ^ {(b} t _ {1})} ^ {G} \mathbf {R}\right) \underset {G} {I ^ {(b} t _ {1})} \mathbf {R} \tag {20}
$$

$$
{ } ^ { G } \mathbf { p } _ { I ( b t _ { k } ) } = ( 1 - \lambda _ { k } ) ^ { G } \mathbf { p } _ { I ( b t _ { 1 } ) } + \lambda _ { k } { } ^ { G } \mathbf { p } _ { I ( b t _ { 2 } ) } \tag {21}
$$

$$
\lambda_ {k} = \frac {{} ^ {i} t _ {k} + {} ^ {i} t _ {b} - {} ^ {b} t _ {1}}{{} ^ {b} t _ {2} - {} ^ {b} t _ {1}} \tag {22}
$$

where $\operatorname{Log}(\cdot)$ is the inverse operation of $\operatorname{Exp}(\cdot)$ which maps a rotation matrix to a vector in $\mathbb{R}^3$ . These equations essentially interpolate both the orientation and position under the approximation of constant linear and angular velocity over the interval. As images of the base camera typically arrive at high rate (around $20\mathrm{Hz}$ in most applications) as compared to the physical camera motion, we argue that this serves as a good approximation. Due to the fact that marginalization is only ever performed on the oldest clone, any required camera pose will typically be within 25 milliseconds of a clone in the sliding window. Note that a similar interpolation scheme was used in our prior work on asynchronous multi-sensor fusion [34] but without either the intrinsic and extrinsic (spatial and temporal) calibration of multiple non-base cameras as focused on in this work.

Let $\mathbf{x}_I({}^b t_1)$ , $\mathbf{x}_I({}^b t_2)$ and $\mathbf{x}_I({}^b t_k)$ denote the IMU clones at the neighboring times, and the interpolated value, respectively. By substituting the above expressions into the measurement function, we have the following Jacobians with respect to the bounding IMU clones and the time offset ${}^i t_b$ :

$$
\frac {\partial^ {C _ {i} \left(^ {b} t _ {k}\right)} \delta \mathbf {p} _ {f}}{\partial \delta \mathbf {x} _ {I} \left(^ {b} t _ {1}\right)} = \frac {C _ {i} \left(^ {b} t _ {k}\right) \delta \mathbf {p} _ {f}}{\partial \delta \mathbf {x} _ {I} \left(^ {b} t _ {k}\right)} \frac {\partial \delta \mathbf {x} _ {I} \left(^ {b} t _ {k}\right)}{\partial \delta \mathbf {x} _ {I} \left(^ {b} t _ {1}\right)} \tag {23}
$$

$$
\frac {\partial^ {C _ {i} \left(^ {b} t _ {k}\right)} \delta \mathbf {p} _ {f}}{\partial \delta \mathbf {x} _ {I} \left(^ {b} t _ {2}\right)} = \frac {C _ {i} \left(^ {b} t _ {k}\right) \delta \mathbf {p} _ {f}}{\partial \delta \mathbf {x} _ {I} \left(^ {b} t _ {k}\right)} \frac {\partial \delta \mathbf {x} _ {I} \left(^ {b} t _ {k}\right)}{\partial \delta \mathbf {x} _ {I} \left(^ {b} t _ {2}\right)} \tag {24}
$$

$$
\frac {\partial^ {C _ {i} \left(^ {b} t _ {k}\right)} \delta \mathbf {p} _ {f}}{\partial \delta^ {i} t _ {b}} = \frac {C _ {i} \left(^ {b} t _ {k}\right) \delta \mathbf {p} _ {f}}{\partial \delta \mathbf {x} _ {I} \left(^ {b} t _ {k}\right)} \frac {\partial \delta \mathbf {x} _ {I} \left(^ {b} t _ {k}\right)}{\partial \delta^ {i} t _ {b}} \tag {25}
$$

To compute these Jacobians, we use the following approximations for a small angle $\psi$ [35]:

$$
\begin{array}{l} \operatorname {E x p} (\boldsymbol {\theta} + \boldsymbol {\psi}) \approx \operatorname {E x p} \left(\mathbf {J} _ {l} (\boldsymbol {\theta}) \boldsymbol {\psi}\right) \operatorname {E x p} (\boldsymbol {\theta}) (26) \\ \approx \operatorname {E x p} (\boldsymbol {\theta}) \operatorname {E x p} \left(\mathbf {J} _ {r} (\boldsymbol {\theta}) \psi\right) (27) \\ \end{array}
$$

where $\mathbf{J}_l(\cdot)$ and $\mathbf{J}_r(\cdot)$ are the left and right Jacobians of $SO(3)$ [30], respectively. By defining for convenience ${}_{1}^{2}\hat{\pmb{\theta}} = \mathrm{Log}\left(\frac{I^{(b}t_{2})}{I^{(b}t_{1})}\hat{\mathbf{R}}\right)$ , we have the following Jacobians with detailed derivations found in our technical report [36]:

$$
\begin{array}{l} \frac {\partial^ {I ^ {(b} t _ {k})} \delta \boldsymbol {\theta} _ {G}}{\partial^ {I ^ {(b} t _ {1})} \delta \boldsymbol {\theta} _ {G}} = - \left(\hat {\lambda} _ {k} \mathbf {J} _ {l} \left(\hat {\lambda} _ {k 1} ^ {2} \hat {\boldsymbol {\theta}}\right) \mathbf {J} _ {r} ^ {- 1} \binom {2} {1} \hat {\boldsymbol {\theta}}\right) - \operatorname {E x p} \left(\hat {\lambda} _ {k 1} ^ {2} \hat {\boldsymbol {\theta}}\right) \\ \frac {\partial^ {I ^ {(b} t _ {k)}} \delta \boldsymbol {\theta} _ {G}}{\partial^ {I ^ {(b} t _ {2})} \delta \boldsymbol {\theta} _ {G}} = \hat {\lambda} _ {k} \mathbf {J} _ {l} \left(\hat {\lambda} _ {k 1} ^ {2} \hat {\boldsymbol {\theta}}\right) \mathbf {J} _ {l} ^ {- 1} \left(^ {2} _ {1} \hat {\boldsymbol {\theta}}\right) \\ \frac {\partial^ {I ^ {(b} t _ {k})} \delta \boldsymbol {\theta} _ {G}}{\partial \delta^ {i} t _ {b}} = - \frac {1}{^ b t _ {2} - ^ {b} t _ {1}} ^ {2} \hat {\boldsymbol {\theta}} \\ \frac {\partial^ {G} \delta \mathbf {p} _ {I \left(^ {b} t _ {k}\right)}}{\partial^ {G} \delta \mathbf {p} _ {I \left(^ {b} t _ {1}\right)}} = (1 - \hat {\lambda} _ {k}) \mathbf {I}, \quad \frac {\partial^ {G} \delta \mathbf {p} _ {I \left(^ {b} t _ {k}\right)}}{\partial^ {G} \delta \mathbf {p} _ {I \left(^ {b} t _ {2}\right)}} = \hat {\lambda} _ {k} \mathbf {I} \\ \frac {\partial^ {G} \delta \mathbf {p} _ {I \left(^ {b} t _ {k}\right)}}{\partial \delta^ {i} t _ {b}} = \frac {1}{b t _ {2} - ^ {b} t _ {1}} \left(^ {G} \hat {\mathbf {p}} _ {I \left(^ {b} t _ {2}\right)} - ^ {G} \hat {\mathbf {p}} _ {I \left(^ {b} t _ {1}\right)}\right) \tag {28} \\ \end{array}
$$
...
![](images/f96f1a095971e7eba2d44b0904efb8764e7cdc60d2b53e7823ea05c8b0ea7374.jpg)  
...
![](images/f4876b329d173e9a9885f4d89df6b0d98669394a5a0362aead1d3edab6ccf9b6.jpg)  
...
[11] G. Huang, K. Eckenhoff, and J. Leonard. "Optimal-State-Constraint EKF for Visual-Inertial Navigation". In: Proc. of the International Symposium on Robotics Research. Sestri Levante, Italy, Sept. 2015.   
...
[26] C. Hertzberg, R. Wagner, U. Frese, and L. Schröder. "Integrating generic sensor fusion algorithms with sound state representations through encapsulation of manifolds". In: Information Fusion 14.1 (2013), pp. 57-77.   
...
[34] P. Geneva, K. Eckenhoff, and G. Huang. "Asynchronous Multi-Sensor Fusion for 3D Mapping and Localization". In: Proc. of the IEEE International Conference on Robotics and Automation. 2018.   
...
[36] K. Eckenhoff, P. Geneva, J. Bloecker, and G. Huang. Multi-Camera Visual-Inertial Navigation with Online Intrinsic and Extrinsic Calibration. Tech. rep. RPNG-2018-MCVINS. Available: http://udel.edu/\~ghuang/papers/tr_mc-vins.pdf. University of Delaware, 2018.